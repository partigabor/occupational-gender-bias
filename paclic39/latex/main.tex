...
\begin{abstract}
This paper is about occupational gender bias and gender stereotypes, presented in a cross-linguistic, cross-cultural setting. In the study, we analyze experimental data collected from Hungarian and Chinese speakers on their ratings of occupations, answering a question on how typically a job is done by either men or women. Results show that in both of these languages the words carry societal biases, despite that the job titles themselves have no gender markings. We compare the ratings across linguistic and gender lines, highlight the differences, and discuss the results with insights ranging from peculiarities in word formation to more generic societal differences. We also compare the human raters' responses with that of a few popular generative AI engines, which will show that the biases we humans carry are even stronger in the Large Language Models (LLMs) underlying these chatbots.
\end{abstract}
...
For both Hungarian and Chinese, we collected data from 20 participants. After validating the responses, the Hungarian dataset had 11 women and 9 men, with ages ranges of 25-35 (n=11), 35-45 (n=4), and 45-55 (n=5). See Figure~\ref{fig:demographics_hu} for the distribution.
...
% , each containing around \hl{42} commonly occurring job titles. Participants had to rate each word on a 7-point Likert scale, according to how typically they perceive them as jobs rather done by men or by women. In both questionnaires the scales were presented to the participants as follows: completely male; mostly male; more/somewhat male; neutral/equal; more/somewhat female; mostly female; completely female. 
...
We devised two simple experiments, where we asked participants to rate job titles on a 7-point Likert scale, according to ``how typically male or female jobs'' they represent, how likely that occupation is to be done by men or women. 
