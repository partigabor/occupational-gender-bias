...
Additionally, we compared the human raters' responses with that of a few popular generative AI agents. Interestingly, the biases exhibited by the LLMs in these chatbots were found to be even stronger than those shown by human participants.
 \\ \newline \Keywords{gender bias, workplace, occupations, Hungarian, Chinese, LLMs} }

% Excluding any number of additional pages for acknowledgements, references, ethical considerations, limitations, conflict-of-interest, as well as data and code availability statements.
...
%%% FOR FINAL VERSION
and 6 attention-check items, each a well-known occupational title in Hungary, such as: \textit{modell} `model' or \textit{katona} `soldier'.\footnote{See the items \href{https://anonymous.4open.science/r/occupational-gender-bias/occupations.xlsx}{here}.} The attention-check items were removed from the final analysis, these were \textit{pincérnő} `waitress', \textit{titkárnő} `secretary (female)', \textit{tanárnő} `teacher (female)', \textit{takarítónő} `cleaning lady', \textit{ápolónő} `nurse (female)', and \textit{házvezetőnő} `housekeeper (female)'. These words explicitly determine the gender of the worker and all should be rated `completely female' (3). Participants who rated any of these lower than 2, or rated them lower than 3 more then once were rejected.
