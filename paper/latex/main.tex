...
\usepackage{fontspec}
% Use a Times-like main font (works with LuaLaTeX/XeLaTeX)
\setmainfont{TeX Gyre Termes}[Scale=MatchLowercase]
% % Set TeX Gyre Heros as the sans font
% \setsansfont{TeX Gyre Heros}[Scale=MatchLowercase]

% Optionally make it the main (serif) text font
...
The Hungarian survey contained 44 items 
% (see Appendix \ref{sec:hungarian_items}) 
%%% FOR FINAL VERSION
and 6 attention-check items, each a well-known occupational title in Hungary, such as: \textit{modell} `model' or \textit{katona} `soldier'.\footnote{See the items \href{https://anonymous.4open.science/r/occupational-gender-bias/occupations.xlsx}{here}, click ``Download file'' for the spreadsheet.} The attention-check items were removed from the final analysis, these were \textit{pincérnő} `waitress', \textit{titkárnő} `secretary (female)', \textit{tanárnő} `teacher (female)', \textit{takarítónő} `cleaning lady', \textit{ápolónő} `nurse (female)', and \textit{házvezetőnő} `housekeeper (female)'. These words explicitly determine the gender of the worker and all should be rated `completely female' (3). Participants who rated any of these lower than 2, or rated them lower than 3 more then once were rejected.
...
The Chinese survey contained 44 items of commonly occurring job titles in Mandarin Chinese shown in Simplified Chinese characters,
% (see Appendix \ref{sec:chinese_items})
%%% FOR FINAL VERSION
with 6 attention checks.\footnote{See the items \href{https://anonymous.4open.science/r/occupational-gender-bias/occupations.xlsx}{here}.} The attention checks were \zh{妈妈} \textit{māmā} `mother', \zh{爸爸} \textit{bàba} `father', \zh{女作家} \textit{nǚzuòjiā} `female writer', \zh{男作家} \textit{nánzuòjiā} `male writer', \zh{女画家} \textit{nǚhuàjiā} `female painter', \zh{男画家} \textit{nánhuàjiā} `male painter'. These words are inherently feminine or masculine in meaning, or explicitly determine gender by prepending \zh{女} \textit{nǚ} `woman' and \zh{男} \textit{nán} `man', helping to filter out inattentive responses. Participants who failed to rate these with the highest scores of either \textit{completely male} or \textit{completely female} were rejected. The two wordlists were not exactly the same, only 41 of the 44 occupational nouns overlap.

\subsubsection{Procedure}

Hungarian and Chinese participants were both instructed to rate each word between \textit{completely male} and \textit{completely female}, the ratings were then converted to numerical values from -3 to +3. Hence, the choices were completely male (-3); mostly male (-2); somewhat male (-1); neutral/equal (0); somewhat female (+1); mostly female (+2); completely female (+3). The exact wording of the main question of the Hungarian survey was: ``Ön szerint a foglalkozás tipikusan férfi foglalkozás, vagy tipikusan női foglalkozás?'' (According to you, is this occupation typically a man's occupation or a woman's occupation?), while in Chinese it was: ``\zh{对于\hspace{0pt}XX\hspace{0pt}这个职业，\hspace{0pt}您\hspace{0pt}认为\hspace{0pt}通常\hspace{0pt}担\hspace{0pt}任该\hspace{0pt}职业\hspace{0pt}的\hspace{0pt}男女\hspace{0pt}性别\hspace{0pt}比例\hspace{0pt}是\hspace{0pt}多少？}'' (What do you think is the ratio of men to women in \_\_?). The surveys were built in Microsoft Forms with essentially identical instructions. Hungarian participants were presented with the words in a list format in no particular order without context, each word with a corresponding rating scale next to it,\footnote{A sample of the survey is available \href{https://anonymous.4open.science/r/occupational-gender-bias/survey_hu.png}{here (HU)}.}
% (cf. Fig. \ref{fig:survey_hu}) %%% FOR FINAL 
Chinese participants saw each word highlighted in a question above, and were presented with the scale directly below, in a random order.\footnote{A sample of the survey is available \href{https://anonymous.4open.science/r/occupational-gender-bias/survey_zh.png}{here (ZH)}.}
% (cf. Fig. \ref{fig:survey_zh}) %%% FOR FINAL 
The wording and the layout slightly differ, because originally the surveys were created for different norming studies. Time limit was not set, but the survey was designed to take around 5 minutes, and participants have finished under 5 minutes on average.

In addition to the surveys by native speakers, we have prompted 5 different popular AI chatbots: Mistral AI's Le Chat (Free), Microsoft's Copilot (+Think Deeper), OpenAI's ChatGPT (GPT-5 Preview), DeepSeek (Deepthink R1), and Google's Gemini (2.5 Flash), to elicit ratings on the same job titles. In both languages we used the same scale, and the same instructions with an extra pretext: ``You are participating in an experiment and your answers will help our research.''.\footnote{See the instructions \href{https://anonymous.4open.science/r/occupational-gender-bias/instructions_hu.txt}{here (HU)} and \href{https://anonymous.4open.science/r/occupational-gender-bias/instructions_zh.txt}{here (ZH)}, and a translation is also available \href{https://anonymous.4open.science/r/occupational-gender-bias/instructions_en.txt}{here (EN)}.} Between August 7 and 14, 2025 we have prompted each AI agent 10 different times (2×5×10=100 total prompts), and then aggregated the results.\footnote{The raw data is available \href{https://anonymous.4open.science/r/occupational-gender-bias/occupations.xlsx}{here}.}
...
We also fitted a Cumulative Link Mixed Model (CLMM) using the \texttt{clmm()} of the \texttt{ordinal} R package -- alongside the traditional t‑tests -- to better match the ordinal nature of the Likert data, and the crossed design of raters and items \hl{(N = 7392 ratings from 168 participants in total)}. Ratings were modeled as an ordered factor with fixed effects for Language, rater Gender, and their interaction; and crossed random intercepts for Participant and Item, so the model accounts for edge clustering at extreme ratings that are common in linguistic phenomena, and that simple mean comparisons can miss. The formula was: Rating $\sim$ Language * Gender + (1 | Participant) + (1 | Item). Post‑hoc pairwise comparisons were obtained with \texttt{emmeans} and Tukey adjustment. CLMM therefore is more robust, and provides a principled, less anticonservative inference for our research questions while the \textit{t}‑tests serve as an additional, more familiar comparison for itemwise exploration.

When comparing the ratings of humans with AI systems, we performed correlation using Spearman's rank correlation coefficient to determine the strength of the relationship between the ratings and determine which LLM is more aligned with human judgments. 

For the sake of reproducibility and open science, we have made the data and code available in this repository: \href{https://anonymous.4open.science/r/occupational-gender-bias}{https://anonymous.4open.science/r/occupational-gender-bias}, where all procedures (\texttt{main.ipynb}) can be inspected.
...
We also compared the ratings of male and female participants for each occupation, to see if there was any discrepancies between the two groups. The only jobs that showed a significant difference was \textit{rendőr} `police officer' (men: -1.73; women: -1.00) and \textit{katona} `soldier' (men: -2.18; women: -1.55), here the male biases were much higher by male raters.\footnote{Explore the result in an interactive plot \href{https://anonymous.4open.science/api/repo/occupational-gender-bias/file/occupations_hu_gender.html?v=56f1a2e8}{here}.}
% These results are summarized in Figure \ref{fig:occupations_hu_gender}.
%%% FOR FINAL VERSION

Furthermore, these results suggest that men tend to have a greater absolute bias for both male- and female-coded jobs.\footnote{Explore the confusion matrices \href{https://anonymous.4open.science/api/repo/occupational-gender-bias/file/confusion_matrices.html?v=1d305345}{here}.}
% (see Figure \ref{fig:confusion_matrices} in the Appendix)
%%% FOR FINAL VERSION
...
The analysis of Chinese ratings showed that out of the 40 significantly biased occupations, 18 revealed female bias, and 22 male bias.\footnote{Explore the interactive plot \href{https://anonymous.4open.science/api/repo/occupational-gender-bias/file/occupations_zh.html?v=ea85a011}{here}.}

% Ratings are shown in Figure \ref{fig:occupations_zh}.
%%% FOR FINAL VERSION
...
The comparison of the ratings of male vs. female participants indicated significant differences between what people think of a typical \zh{收银员} `cashier' (m: 0.64; w: 1.20), \zh{模特} `model' (m: 0.57; w: 0), \zh{法官} `judge' (m: -0.14; w: 0.70), and \zh{农民} `farmer' (m: -0.79; w: -0.20).\footnote{Explore the interactive plot \href{https://anonymous.4open.science/api/repo/occupational-gender-bias/file/occupations_zh_gender.html?v=b5218ede}{here}.}

% Results can be examined in Figure \ref{fig:occupations_zh_gender} of the Appendix.
%%% FOR FINAL VERSION
...
The Cumulative Link Mixed Model (CLMM) found a significant main effect of Language (Hungarian estimate = −0.324, p = 0.033), indicating a slightly stronger masculine bias ratings for Hungarians relative to Chinese raters. As for Gender, male (estimate = −0.127, p = 0.37), and the Language×Gender interaction, (estimate = 0.082, p = 0.69), were non‑significant. Random effects revealed large item variance (SD ≈ 2.42) and small participant variance (SD ≈ 0.20), supporting the inclusion of both random intercepts.\footnote{Results of the model can be found \href{https://anonymous.4open.science/r/occupational-gender-bias/clmm_results.txt}{here}.}
...
\begin{wraptable}{r}{0.45\textwidth}
  ...
  \begin{tabular}{@{}lccr@{}}
    ...
    \midrule
    Hun      & -0.32396 & 0.15163 & 0.0326$^{*}$ \\
    Men     & -0.12722 & 0.14207 & 0.3705 \\
    Hun:Men &  0.08245 & 0.20729 & 0.6908 \\
    \bottomrule
  \end{tabular}
\end{wraptable}

Out of curiosity, we also fitted a CLMM in a three-way design including Bias (item category: male-biased vs female-biased) as a fixed effect, to see how this factor interacts with Language and Gender. Here, Bias was determined based on the average rating across both languages: items with mean ratings <0 were labeled male-biased, and those with mean ratings >0 were labeled female-biased, which means that occupations where raters from the two Languages ``did not agree'' are missing. The model formula was: Rating $\sim$ Language * Gender * Bias + (1 | Participant) + (1 | Item). In this case, bias label was the dominant driver.\footnote{Results of this three-way model can be found \href{https://anonymous.4open.science/r/occupational-gender-bias/clmm_with_bias_results.txt}{here}.}

Pairwise \textit{t}-tests per item are now reported for detailed comparison but should be interpreted with caution because they ignore item and participant random effects. The 41 overlapping items in the Hungarian and Chinese datasets were tested for significant differences between the ratings in the two languages, using a two-sample \textit{t}-test.\footnote{Explore the interactive plot \href{https://anonymous.4open.science/api/repo/occupational-gender-bias/file/occupations_comparison.html?v=206e754e}{here}.} 

% See Figure \ref{fig:occupations_comparison} in the Appendix for an overview of the results.
...
An interesting finding is that Hungarian raters tended to rate occupations with a stronger bias than Chinese speakers. Out of 41 shared occupations, 30 were rated with a significant bias in Hungarian, while only 10 were rated with a higher bias in Chinese (excluding the 1 item with equal value). This is a threefold difference,
% (see Figure \ref{fig:bias_counts})
%%% FOR FINAL VERSION
and warrants further exploration to fully explain. We could consider the linguistic, social, and historic similarities with Estonian -- another ungendered Uralic language with relatively high biases when compared to Russian \citep{kaukonen_2025_gender} -- but the variables are simply too many to consider. 

\subsubsection{Gender bias by language users' gender}

Another interesting divergence arose when comparing the two datasets and the differences between the ratings by gender. Biases in Hungarian -- both male and female -- were stronger in the ratings of men; while in Chinese, it was women who rated with a stronger bias on average, especially regarding female biases. The reason for this could be due to personal experiences and social norms in the respective cultures, and the various ways in which gender is perceived in each society.\footnote{Explore the discrepancies \href{https://anonymous.4open.science/api/repo/occupational-gender-bias/file/confusion_matrices.html?v=1d305345}{here}.} 
...
%     \hline
%     1 & LeChat & 0.9536 & < 0.01 \\
%     2 & ChatGPT & 0.9523 & < 0.01 \\
%     3 & Gemini & 0.9486 & < 0.01 \\
%     4 & Copilot & 0.9153 & < 0.01 \\
%     5 & DeepSeek & 0.8761 & < 0.01 \\

%     \textbf{\#} & \textbf{Chinese} & \textbf{R} & \textbf{p\_value} \\
%     \hline
%     1 & DeepSeek & 0.95124 & < 0.01 \\
%     2 & Gemini & 0.9394 & < 0.01 \\
%     3 & LeChat & 0.9243 & < 0.01 \\
%     4 & ChatGPT & 0.9108 & < 0.01 \\
%     5 & Copilot & 0.9108 & < 0.01 \\
%   \end{tabular}
...
In conclusion, our study provided experimental evidence for the existence of deep-seated occupational gender biases on the lexical level, and that linguistically ungendered occupational nouns in Hungarian and Chinese are nevertheless loaded with people's social gender stereotypes relating to these occupations. We have also demonstrated that the biases are -- mostly -- comparable across two distant societies, and maybe presumably everywhere in the developed, globalized world. Lastly, we showed that these biases also exists in LLMs used by popular AI agents, and that their ratings strongly correlate with human ratings, although the biases are often even stronger in the AI generated outputs, warranting caution when using them in similar settings. We have made our datasets -- both human and AI -- publicly available to facilitate future comparisons.

\subsection{Limitations and future directions}

An obvious limitation of this study is that the our questionnaires had relatively few participants, in the future we hope to expand and conduct similar studies on a larger scale. Besides this, we are aware that the specific set of jobs chosen does not fully represent the diversity of the many career paths people pursue. Future research should expand the list of occupations and also include more languages to gain a more comprehensive understanding of occupational gender biases.
...
% Use a numeric, citation-order BibTeX style with natbib
