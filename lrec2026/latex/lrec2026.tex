...
We also fitted a Cumulative Link Mixed Model (CLMM) using the \texttt{clmm()} of the \texttt{ordinal} R package -- alongside the traditional t‑tests -- to better match the ordinal nature of the Likert data, and the crossed design of raters and items (N = 2024 ratings from 46 participants and 42 items). Ratings were modeled as an ordered factor with fixed effects for Language, rater Gender, and their interaction; and crossed random intercepts for Participant and Item, so the model accounts for repeated measures, item heterogeneity, but especially edge clustering at extreme ratings that are common in linguistic phenomena, and that simple mean comparisons can miss. The formula was: Rating $\sim$ Language * Gender + (1 | Participant) + (1 | Item). The model used a logit link and the Laplace approximation for likelihood estimation. Post‑hoc pairwise comparisons were obtained with \texttt{emmeans} and Tukey adjustment. CLMM therefore is more robust, and provides a principled, less anticonservative inference for our research questions while the t‑tests serve as an additional, more familiar comparison for itemwise exploration.
