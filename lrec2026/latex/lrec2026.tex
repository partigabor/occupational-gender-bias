...
We also fitted a Cumulative Link Mixed Model (CLMM) using the \texttt{clmm()} of the \texttt{ordinal} R package -- alongside the traditional t‑tests -- to better match the ordinal nature of the Likert data, and the crossed design of raters and items (N = 2024 ratings from 46 participants and 42 items). Rating was modeled as an ordered factor with fixed effects for Language, rater Gender, and their interaction; and crossed random intercepts for Participant and Item, so the model accounts for repeated measures, item heterogeneity, but especially edge clustering at extreme ratings that are common in linguistic phenomena, and that simple mean comparisons can miss. The formula was: Rating ~ Language * Gender + (1 | Participant) + (1 | Item) The model used a logit link and the Laplace approximation for likelihood estimation. Post‑hoc pairwise comparisons were obtained with \texttt{emmeans} and Tukey adjustment. CLMM therefore is more robust, and provides a principled, less anticonservative inference for our research questions while the itemwise t‑tests serve as an additional, more familiar comparison for itemwise exploration.

When comparing the ratings of humans with AI systems, we performed Pearson's correlation to determine the strength of the relationship between the ratings and determine which LLM is more aligned with human judgments. 
...
The Cumulative Link Mixed Model (CLMM) found a significant main effect of Language: Hungarian estimate = −0.324, SE = 0.152, z = −2.136, p = 0.033, corresponding to OR = exp(−0.324) = 0.72 (95\% CI ≈ 0.54–0.96), indicating lower odds of higher stereotypicality ratings for Hungarian relative to Chinese raters. As for Gender, male (estimate = −0.127, p = 0.37), and the Language×Gender interaction, (estimate = 0.082, p = 0.69), were non‑significant. Random effects reveal large item variance (SD ≈ 2.42) and small participant variance (SD ≈ 0.20), supporting the inclusion of both random intercepts. Tukey‑adjusted emmeans follow this pattern; no pairwise contrast remains strong after adjustment, implying a modest overall language effect rather than large cellwise differences. 
